...
var rules = {
	showList: function (name, chainTable) {
        ...
        parser.endEditRule();
		http.get("/channel?c=" + channel + "&t=" + table).then(data => {
	...
	remove: function (index) {
		http.get("/delete?i=" + index + "&c=" + channel + "&t=" + table, parser.parseChannels);
		return false;
	},
	...
	monitor: function() {
		http.get("/mon?c=" + channel + "&t=" + table).then((data) => {
			let arr = JSON.parse(data);
			let index = 0;
			
			arr.forEach((item, i) => {
				let items = item.trim().replace(/[ ]+/g, ' ').split(" ");
				if(i === 0) {
					pkts = 4;
					bytes = 6;
				}
